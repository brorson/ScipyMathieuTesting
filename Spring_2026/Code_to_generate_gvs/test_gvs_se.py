...
test_gvs_se.py
==============
Test SciPy's mathieu_sem against golden values (GVs).
...
SciPy's mathieu_sem output at each (m, q, v) grid point, and writes
out any non-passing results.

Directory layout expected:
    <GV_INPUT_DIR>/python_mathieu_se_gvs_q<qval>.csv  -- input GV files
    <RESULTS_OUTPUT_DIR>/not_passed_se_q<qval>.csv     -- output failures

CSV format of GV files:
    - First row: header (skipped)
    - Columns: v, se(m=1), se(m=2), ..., se(m=M_MAX)
    - Rows: 2500 points from v = -pi to pi

Note: Unlike ce, the se function is defined for m >= 1 (se_0 does not
exist), so the column indexing differs — se(m) is at column index m
(no +1 offset needed, since there is no m=0 column).

Usage:
    python test_gvs_se.py
...
# Glob pattern to match se golden value files
GV_FILE_PATTERN = os.path.join(GV_INPUT_DIR, "mathieu_se_gvs_q100.000000*")

# Directory to write non-passing results
RESULTS_OUTPUT_DIR = os.path.join(SCRIPT_DIR, "python_not_passed_se")

# Number of evaluation points in v (from -pi to pi)
N_POINTS = 2500

# Range of orders m to test: m = M_START, M_START+1, ..., M_END-1
# Note: se is defined for m >= 1 (unlike ce which includes m=0)
M_START = 1
M_END = 36  # exclusive upper bound, so tests m=1..35

# Column offset: in the GV CSV, se(m) is at column index (m + COL_OFFSET)
# For se, the first column is v, and since se starts at m=1,
# se(m=1) is at column 1, se(m=2) at column 2, etc.
# So COL_OFFSET = 0 (unlike ce where COL_OFFSET = 1)
COL_OFFSET = 0
...
def extract_q_from_filename(filepath):
    ...
    Expects filenames like: .../python_mathieu_se_gvs_q1.0.csv
...
def test_se_for_file(filepath, v_grid):
    """
    Compare SciPy mathieu_sem against golden values for a single q.
...
def main():
    ...
    print(f"Found {len(matching_files)} golden value file(s) for se.")
    ...
    for filepath in matching_files:
        ...
        try:
            results_df, q = test_se_for_file(filepath, v_grid)
        ...
        output_file = os.path.join(
            RESULTS_OUTPUT_DIR, f"not_passed_se_q{q}.csv"
        )
